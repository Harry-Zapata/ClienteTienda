...
export class VentaComponent implements OnInit{
  ...
  datosDetalleVentas = new MatTableDataSource(this.listaProductosParaVenta);
  ...
  retornarProductosPorFiltro(busqueda: any): Producto[] {
    ...
    return this.listaProductos.filter(item => {
      return item.nombre.toLowerCase().includes(valorBuscado); // Debe retornar true o false
    });
  }
  constructor(
  ...
  productoParaVenta(event: any) {
   ...
   agregarProductoParaVenta() {
     ...
     if(this.productoSeleccionado.stock < _cantidad){
       this._utilidadServicio.mostrarAlerta("No hay Stock suficiente", "Error");
       return;
     }
     ...
     this.datosDetalleVentas = new MatTableDataSource(this.listaProductosParaVenta);
      ...
      this._ventaServicio.registrar(request).subscribe(
        {
          next: (response)=>
          {
            if(response.status){
              ...
              Swal.fire({
                icon: 'success',
                title: 'La Venta Fue Registrada Correctamente',
